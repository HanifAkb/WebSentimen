...
def _fetch_window_tweets(
    ...
    existing_tweet_keys: set[str] | None = None,
    ...
    if seen_tweet_keys is None:
        seen_tweet_keys = set()
    blocked_tweet_keys = set(existing_tweet_keys or [])
    while tweet_count < max_tweets_per_window:
        ...
        for tweet in raw_tweets:
            ...
            if tweet_key in blocked_tweet_keys:
                continue
            if tweet_key in seen_tweet_keys:
                continue

            if min_created_date is not None or max_created_date is not None:
                created_date = _parse_created_at_date(
                    _coalesce(tweet, "CreatedAt", "createdAt", "created_at", "date")
                )
                if created_date is not None:
                    if min_created_date is not None and created_date < min_created_date:
                        continue
                    if max_created_date is not None and created_date > max_created_date:
                        continue
        ...
        if new_tweets_this_page == 0:
            ...
            if duplicate_only_page_count >= max(DUPLICATE_ONLY_PAGE_LIMIT, 12):
                break
        else:
            duplicate_only_page_count = 0
...
def fetch_tweets(
    ...
    on_window_checkpoint: Callable[[dict[str, Any]], None] | None = None,
    ...
    while window_cursor < parsed_end_exclusive:
        ...
        try:
            raw_window_tweets = _fetch_window_tweets(
                ...
                existing_tweet_keys=existing_tweet_keys,
                min_created_date=window_cursor,
                max_created_date=next_window - timedelta(days=1),
            )
        ...
        if on_window_checkpoint is not None:
            on_window_checkpoint(
                {
                    "window_start": since_str,
                    "window_end": until_str,
                    "next_start_date": next_window.isoformat(),
                    "window_kept_count": len(kept_in_window),
